...
//     const [uploadedImages, setUploadedImages] = useState<{ src: string; publicId?: string; deleteToken?: string }[]>([]);
...
//         },
//         onCreate: ({ editor }) => {
//             // Initialize uploadedImages from initial content
//             const initialImages = getImagesFromEditor(editor);
//             setUploadedImages(initialImages);
//         },
//         onTransaction: ({ editor, transaction }) => {
//             const currentImages = getImagesFromEditor(editor);
//             detectImageRemoval(currentImages);
...
//     const getImagesFromEditor = (editor: any): { src: string; publicId?: string }[] => {
//         const images: { src: string; publicId?: string }[] = [];
//         editor.state.doc.descendants((node: any) => {
//             if (node.type.name === "image") {
//                 images.push({
//                     src: node.attrs.src,
//                     publicId: uploadedImages.find(img => img.src === node.attrs.src)?.publicId,
//                 });
//             }
//         });
//         return images;
...
//     // Function to detect image removal and trigger deletion
//     const detectImageRemoval = (currentImages: { src: string; publicId?: string }[]) => {
//         const removedImages = uploadedImages.filter(
//             prevImg => !currentImages.some(currImg => currImg.src === prevImg.src)
//         );

//         removedImages.forEach(image => {
//             handleImageDeletion(image.src, image.publicId, image.deleteToken);
//         });

//         // Update uploadedImages to match current state
//         setUploadedImages(currentImages);
//     };

//     const handleImageDeletion = async (imageUrl: string, publicId?: string, deleteToken?:string) => {
//         const imageData = uploadedImages.find(img => img.src === imageUrl);
//         if (!imageData || !imageData.publicId) return;

//         // try {
//         //     // Example: Delete from Cloudinary using publicId
//         //     await axios.post("/api/delete-image", { publicId: imageData.publicId });
//         //     setUploadedImages(prev => prev.filter(img => img.src !== imageUrl));
...
//             const publicId = response.data.public_id; // Cloudinary returns this
...
//             setUploadedImages(prev => [...prev, { src: imageUrl, publicId, deleteToken }]);
